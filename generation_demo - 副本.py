...
def generate(text):
    fxxk_list = ["没听明白，要不聊点别的如何？", "是世界变化太快，还是我不够有才？为何你说话我不明白？", "恩？你到底在说什么呢？", "好吧，跟骨灰级大师聊天真心感觉压力好大呀，您老人家就不能来点简单点的么~", "看不懂，大侠，能不能换个说法？？？", "唉，你输入的内容真的好深奥呀，能不能说点容易的，给人家留点面子嘛。", "没事耍什么深沉，害我都无法理解，来点简单的呗。", "哈哈哈，不无道理！"]
    r = get_response_candidate(text)
    response_list = []
    n = 0
    for i,j in enumerate(r):
        response_list.append(j[1][1])
        if i == 10:
            break
    if r[0][1][1] < 0.6 or len(text) >= 60:
        return fxxk_list[np.random.randint(len(fxxk_list))]
    else:
        return response_list[np.random.randint(len(response_list))].replace(' ','')